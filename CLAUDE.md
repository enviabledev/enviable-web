...
- **Mirror-painted detail pages: relation-read audit before declaring done.** Any detail page that paints from the mirror reads a typed entity object, but the mirror's raw Prisma rows carry FOREIGN-KEY columns only, not joined relations. The renderer reading `entity.relation.field` (e.g. `payment.paymentMethod.name`, `so.customer.name`, `unit.currentWarehouse.name`) will crash with `Cannot read properties of undefined (reading 'X')` when painted from the mirror unless the relation is explicitly reconstructed in `loadFromMirror`. **Audit step before declaring any detail page done:** grep the render section for every `entity.relation.field` pattern (e.g. `grep -oE '(so|p|l|...)\.[a-zA-Z]+\.[a-zA-Z]+'` over the render code), and for each match verify EITHER (a) the relation is reconstructed in `loadFromMirror` by pulling its mirror bucket and joining by FK, with a sensible fallback when the relation row is missing, OR (b) the read uses optional chaining (`entity.relation?.field ?? fallback`). Either is sufficient; neither is a crash waiting to happen. This is the field-access-audit principle banked elsewhere, sharpened to its concrete check: a grep, a verification per match, no other detail page declared done without it. Bit twice on the SO detail (`so.customer` round 1, `payment.paymentMethod` round 2) because a one-instance fix on the symptom did not sweep the file for the same class; this rule names the sweep as the explicit verification step.
