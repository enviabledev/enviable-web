...
- **Visible-outcome assertions verify rendering correctness; content-shape assertions verify data completeness; both are needed, neither subsumes the other.** A screen that renders an empty column correctly will pass every visible-outcome assertion ("the row rendered, the link worked, the filter narrowed") while silently exposing a content-completeness gap (the column is supposed to be populated and isn't). Caught on the audit-log report: 10 Playwright assertions passed, but every entry's `beforeState` was null because the backend interceptor never captures it, and no assertion was looking. **When the data's completeness is part of the screen's value, add a content-shape assertion alongside the rendering ones**: "an update-action audit entry must have non-null beforeState"; "a confirmed payment must have a non-null confirmedById"; "a fully-received shipment must have receivedAt populated"; etc. These are domain-level assertions distinct from rendering-level ones, and they catch what the rendering checks miss. Recognition: if the screen's primary value depends on a specific field/relation being meaningfully populated (not just present-or-null), a content-shape assertion belongs in the verification set.
...
- **Relation-read audit on reconstructed client-side joins.** Any screen that joins entities client-side from the mirror's separate buckets (detail pages painting `entity.relation.field`, list pages joining `row.fkId` to a sibling bucket's `row.name`, anything else assembling a view across multiple buckets) needs an explicit audit pass before being declared done. The mirror's raw Prisma rows carry foreign-key columns only, not joined relation objects, so `entity.relation.field` crashes with `Cannot read properties of undefined (reading 'X')` unless the relation is explicitly reconstructed. **Audit step:** grep the render section for every `entity.relation.field` pattern (e.g. `grep -oE '(so|p|l|...)\.[a-zA-Z]+\.[a-zA-Z]+'`), and for each match verify EITHER (a) the relation is reconstructed in the mirror read by pulling its bucket and joining by FK (with a sensible fallback when the related row is missing — a missing lookup returns null, not a crash; null is handled honestly in the render with `--` or a similar marker, not silently rendered as empty string), OR (b) the read uses optional chaining (`entity.relation?.field ?? fallback`). The same audit applies to list-page row reconstructions where each row joins to sibling buckets (PI list joining `pi.purchaseOrderId -> po -> counterparty`, invoices+payments list joining `pmt.salesOrderId -> so -> customer`). Field-access audit + relation-lookup-safety together: every rendered field is sourced from a reconstructed-or-guarded value, and every lookup whose result is consumed handles the null case explicitly. Bit twice on the SO detail (`so.customer` round 1, `payment.paymentMethod` round 2) because a one-instance fix did not sweep the file; this rule names the sweep as the explicit verification step.
